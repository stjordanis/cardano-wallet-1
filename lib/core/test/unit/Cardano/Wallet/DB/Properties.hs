    ...
    prop = forAll (genBatches args) $ \(Batches batches) ->
        ...
        in
            head allDBs === SparseCheckpointsDB (sparseCheckpoints cfg tip)
                .&&.
                conjoin (map (alwaysTrue (reverse allDBs)) $ debugIndices $ reverse allDBs)
      where
        debugIndices xs = map (first (,length xs)) $ zip [0..] xs

    -- Postcondition for all intermediate steps
    alwaysTrue :: [SparseCheckpointsDB] -> ((Int, Int), SparseCheckpointsDB) -> Property
    alwaysTrue dbs ((n,tot), SparseCheckpointsDB cps) =
        fromIntegral (edgeSize cfg) < epochStability cfg ==> not (null cps) ==> do
        let tip = last cps
        let k = epochStability cfg
        let g = gapSize cfg
        let firstUnstable = tip - fromIntegral (edgeSize cfg)
        let older = if tip > k + g then tip - k - g else 0
        let beforeK = filter (\x -> x <= firstUnstable && x >= older) cps
